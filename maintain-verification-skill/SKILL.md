...
description: "Periodic pass that keeps a project's verification skill and feature map honest: parallel source readers per feature, one live session driving every feature, at most one PR of proven corrections. Use for /maintain-verification-skill, $maintain-verification-skill, or \"audit the verify skill\"."
---
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.agents/skills/verify-*/`, linked from `.claude/skills/verify-*/`; a link and its target are one candidate). Several candidates → ask which one; none → stop and point at the create-verification-skill skill instead of inventing a target.
