...
description: "Generate a project-local verification skill that drives your app the way a user does — any language, framework, or platform. Use for /create-verification-skill or $create-verification-skill, \"make a verify skill for this repo\", or when a project has no scripted way to prove UI/CLI/service behavior."
---

# Create a verification skill

Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates that as a project-local skill (`verify-<app>/`) tailored to the repo, readable by both Claude Code and Codex. You write the generator's output for the next agent, not for a human: it will be read cold, mid-task, by an agent that has never seen the app.
...
Write `.agents/skills/verify-<app>/SKILL.md`, then link it for Claude Code with `ln -s ../../.agents/skills/verify-<app> .claude/skills/verify-<app>` (Codex reads `.agents/skills/`, Claude Code reads `.claude/skills/`). If the repo already keeps its skills in one of those directories, put the real folder there and link the other. Give it YAML frontmatter with only `name: verify-<app>` and a `description` that names the app, the surface, and when to reach for it — without frontmatter the skill never registers. Keep the body agent-neutral: name commands and scripts, not one agent's tool names. Then write these sections, each grounded in what the interview actually found (no placeholders left):
...
Point the user at the maintain-verification-skill skill (`/maintain-verification-skill` in Claude Code, `$maintain-verification-skill` in Codex) for keeping the map honest as the app changes. Suggest a cadence only if they ask.

---

Adapted from [pstack](https://github.com/cursor/plugins/tree/12d587d/pstack) by Lauren Tan ([@poteto](https://x.com/poteto)). MIT, see [LICENSE](LICENSE).
